...
//  iosApp
//
//  Created by Gamatechno Indonesia on 06/10/22.
    ...
    @MainActor class NoteDetailViewModel: ObservableObject {
        ...
        func saveNote(onSaved: @escaping () -> Void) {
            noteDataSource?.insertNote(
                note: Note(id: noteId == nil ? nil : KotlinLong(value: noteId!), title: self.noteTitle, content: self.noteContent, colorHex: self.noteColor, createdAt: DateTimeUtil().now()), completionHandler: { error in
                    onSaved()
                })
        }
